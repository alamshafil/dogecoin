  ...
  if test x$bitcoin_qt_got_major_vers = x5; then
    ...
    if test x$bitcoin_cv_static_qt = xyes; then
      ...
      if test "x$bitcoin_cv_need_acc_widget" = "xyes"; then
        _BITCOIN_QT_CHECK_STATIC_PLUGIN([AccessibleFactory], [-lqtaccessiblewidgets])
      fi
    ...
    if test x$TARGET_OS = xwindows; then
      AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([qcncodecs], [-lqcncodecs])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([qjpcodecs], [-lqjpcodecs])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([qtwcodecs], [-lqtwcodecs])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([qkrcodecs], [-lqkrcodecs])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([AccessibleFactory], [-lqtaccessiblewidgets])
      _BITCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsPrinterSupportPlugin], [-lwindowsprintersupport])
    fi
  fi
  ...
  if test x$bitcoin_qt_got_major_vers = x5; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       ...
       if test x$TARGET_OS = xlinux; then
         QT_LIBS="$QT_LIBS -lfreetype -lfontconfig"
         ...
         PKG_CHECK_MODULES([QTPRINT], [Qt5PrintSupport], [QT_LIBS="-lQt5PrintSupport $QT_LIBS"])
       elif test "$TARGET_OS" = xwindows; then
         PKG_CHECK_MODULES([QT_WINDOWSUIAUTOMATION], [Qt5WindowsUIAutomationSupport], [QT_LIBS="-lQt5WindowsUIAutomationSupport $QT_LIBS"])
       fi
     ])
     else
       if test "x$TARGET_OS" = xwindows; then
         ...
         if test "x$bitcoin_cv_need_platformsupport" = xyes; then
           if test x$bitcoin_cv_qt58 = xno; then
             ...
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FontDatabaseSupport],[main],[QT_LIBS="-lQt5FontDatabaseSupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXFontDatabaseSupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}EventDispatcherSupport],[main],[QT_LIBS="-lQt5EventDispatcherSupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXEventDispatcherSupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}ThemeSupport],[main],[QT_LIBS="-lQt5ThemeSupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXThemeSupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FbSupport],[main],[QT_LIBS="-lQt5FbSupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXFbSupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}DeviceDiscoverySupport],[main],[QT_LIBS="-lQt5DeviceDiscoverySupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXDeviceDiscoverySupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}AccessibilitySupport],[main],[QT_LIBS="-lQt5AccessibilitySupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXAccessibilitySupport not found)))
             BITCOIN_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}WindowsUIAutomationSupport],[main],[QT_LIBS="-lQt5WindowsUIAutomationSupport $QT_LIBS"],BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXWindowsUIAutomationSupport not found)))
             QT_LIBS="$QT_LIBS -lversion -ldwmapi -luxtheme -lnetapi32 -luserenv"
           fi
         fi
       fi
     fi
